...
unsigned int linked_list_length(linked_list_t list) {
	unsigned int ret = 0;
	linked_list_element_t* head = list._start;
	while(head != NULL) head = head->_next, ret++;
	return ret;
}
...
static linked_list_element_t* linked_list_element_copy(linked_list_element_t* list_element) {
	if(list_element == NULL)
		...
		ret->_next = linked_list_element_copy(list_element->_next);
...
void linked_list_connect(linked_list_t* list_base, linked_list_t list2) {
	if(list_base->_start == NULL)
		list_base->_start = list2._start;
	else {
		linked_list_element_t* head = list_base->_start;
...
void linked_list_element_function(linked_list_t list, void (*function)(void*, void*), void* user_data) {
	linked_list_element_t* head = list._start;
	while(head != NULL) {
		function(head->_data, user_data);
		head = head->_next;
	}
}

unsigned int linked_list_find_first(linked_list_t list, int (*function)(void*, void*), void* user_data) {
	unsigned int i = 0;
	linked_list_element_t* head = list._start;
	while(head != NULL) {
		if(function(head->_data, user_data))
			return i;
		head = head->_next;
		i++;
	}
